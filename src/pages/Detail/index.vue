          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="spuSaleAttr in spuSaleAttrList" :key="spuSaleAttr.id">
                ...
                <dd changepirce="0" @click="changeActive(spuSaleAttrValue,spuSaleAttr.spuSaleAttrValueList,spuSaleAttrValue.baseSaleAttrId)" :class="{active: spuSaleAttrValue.isChecked == 1}" v-for="spuSaleAttrValue in spuSaleAttr.spuSaleAttrValueList" :key="spuSaleAttrValue.id">
...
export default {
  ...
  methods: {
    changeActive(value, list, baseId) {
      list.forEach(item => {
        item.isChecked = 0
      })
      value.isChecked = 1
      if (baseId == 1) this.saleAttrColor = value.saleAttrValueName

      if (baseId == 2) this.saleAttrEdition = value.saleAttrValueName
    },
    changeSkuNum(e) {
      // 用户输入进来的文本
    ...
    async addShopcar() {
      ...
      try {
        ...
        //本地存储|会话存储，一般存储的是字符串 可以通过JSON.stringify()转成字符串后存储
        sessionStorage.setItem('SKUINFO', JSON.stringify(this.skuInfo))
        this.$router.push({
          name: 'addCartSuccess',
          query: {
            skuNum: this.skuNum,
            saleAttrColor: this.saleAttrColor,
            saleAttrEdition: this.saleAttrEdition
          }
        })
      } catch (error) {
        alert(error.message)
