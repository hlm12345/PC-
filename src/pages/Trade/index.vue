  ...
  <div class="trade-container">
    ...
    <div class="content">
      ...
      <div class="address clearFix" v-for="address in addressList" :key="address.id">
        <span class="username" :class="address.isDefault == 1 ? 'selected' : ''">{{address.consignee}}</span>
      ...
      <div class="detail">
        ...
        <ul class="list clearFix" v-for="detail in orderInfo.detailArrayList" :key="detail.skuId">
          <li>
            <img :src="detail.imgUrl" style="width:100px;height:100px" alt="">
          </li>
    ...
    <div class="trade">
      ...
      <div class="receiveInfo">
        寄送至:
        <span>{{recevInfo.fullAddress}}</span>
        收货人：<span>{{recevInfo.consignee}}</span>
        <span> {{recevInfo.phoneNum}}</span>
      </div>
    </div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch('getUsrAddList')
  ...
  methods: {
    ...
    async submitOrder() {
      ...
      let { consignee, phoneNum, fullAddress } = this.recevInfo
      let data = {
        consignee: consignee, //最终收件人的名字
        consigneeTel: phoneNum, //最终收件人的电话
